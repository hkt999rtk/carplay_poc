...
INCLUDES += -I../../../../../../ameba_gateway
INCLUDES += -I../../../../../../wsh264/include
INCLUDES += -I../../../../../../3rd_party/chacha
...
SRC_C += ../../../../../../ameba_gateway/example_gateway_ameba.c
SRC_C += ../../../../../../wsh264/src/gateway_proto.c
SRC_C += ../../../../../../wsh264/src/crypto_stream.c
SRC_C += ../../../../../../wsh264/src/base64.c
SRC_C += ../../../../../../3rd_party/chacha/chacha.c
...
GCCFLAGS =
GCCFLAGS += -march=armv8-m.main+dsp -mthumb -mcmse -mfloat-abi=softfp -mfpu=fpv5-sp-d16 -g -gdwarf-3 -Os -c -MMD
...
CFLAGS += -DGATEWAY_AMEBA_ASSUME_WIFI_SAMPLE=1 -DGATEWAY_AMEBA_ENABLE_USB=1

CPPFLAGS = $(GCCFLAGS)
CPPFLAGS += -std=c++11 -fno-use-cxa-atexit
CPPFLAGS += -Wall -Wpointer-arith -Wundef -Wno-write-strings -Wno-maybe-uninitialized
CPPFLAGS += -DGATEWAY_AMEBA_ASSUME_WIFI_SAMPLE=1 -DGATEWAY_AMEBA_ENABLE_USB=1
...
$(ASM_O): %.o : %.S
	...
	cp $@ $(OBJ_DIR)/$(notdir $@)
	if [ -f "$(notdir $*.s)" ]; then mv -f "$(notdir $*.s)" $(INFO_DIR) || true; fi
	chmod 777 $(OBJ_DIR)/$(notdir $@)

$(CPP_O): %.o : %.cpp
	$(CC) $(CPPFLAGS) $(INCLUDES) -c $< -o $@
	cp $@ $(OBJ_DIR)/$(notdir $@)
	if [ -f "$(notdir $*.i)" ]; then mv -f "$(notdir $*.i)" $(INFO_DIR) || true; fi
	if [ -f "$(notdir $*.s)" ]; then mv -f "$(notdir $*.s)" $(INFO_DIR) || true; fi
	chmod 777 $(OBJ_DIR)/$(notdir $@)
