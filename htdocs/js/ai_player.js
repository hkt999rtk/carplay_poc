...
class AIVideo extends Player
{
    ...
    renderFrameSource(source, frameWidth, frameHeight) {
        for (let i=0; i<this.video.length; i++) {
            let output_canvas = document.getElementById(this.video[i]);
            let destCtx = output_canvas.getContext('2d');
            destCtx.imageSmoothingEnabled = true;
            destCtx.drawImage(source, 0, 0, frameWidth,
                frameWidth*output_canvas.height/output_canvas.width, 0, 0,
                output_canvas.width, output_canvas.height);

            let scale = output_canvas.width / frameWidth;
            if (typeof this.onImageReady == "function") {
                this.onImageReady(output_canvas, scale);
            }
        }
...
function _appendWithStartCode(payload) {
    let out = new Uint8Array(payload.length + 4);
    out[0] = 0;
    out[1] = 0;
    out[2] = 0;
    out[3] = 1;
...
function _mergeNalsWithStartCode(nalArray) {
    let total = 0;
    for (let i=0; i<nalArray.length; i++) {
        total += nalArray[i].length + 4;
    }
...
class NativeH264Decoder {
    constructor(videoPlayer, fps, onFallback) {
        ...
        this.pps = null;
        this.timestamp = 0;
        this.frameIntervalUs = Math.floor(1000000 / this.fps);
        this.ready = false;
        this.failed = false;
        this.decoder = new VideoDecoder({
            output: (frame) => {
                if (this.videoPlayer != null && typeof this.videoPlayer.renderNativeFrame == "function") {
                    this.videoPlayer.renderNativeFrame(frame);
                }
                frame.close();
            },
            error: (err) => {
                console.log("WebCodecs decoder error:", err);
                this.fail("decoder error");
            }
        });
    }
    ...
    tryConfigure() {
        if (this.ready || this.failed || this.sps == null || this.pps == null) {
            return;
        }
        if (this.sps.length < 4) {
            this.fail("invalid sps");
            return;
        }
        const codec = "avc1." + _hex2(this.sps[1]) + _hex2(this.sps[2]) + _hex2(this.sps[3]);
        try {
            ...
            this.ready = true;
            console.log("WebCodecs enabled, codec =", codec);
        } catch (e) {
            console.log("WebCodecs configure failed:", e);
            this.fail("unsupported codec");
        }
    }

    pushNal(nal) {
        if (this.failed || nal == null || nal.length == 0) {
            return false;
        }
        ...
        let payload;
        if (nalType == 5 && this.sps != null && this.pps != null) {
            payload = _mergeNalsWithStartCode([this.sps, this.pps, nal]);
        } else {
            payload = _appendWithStartCode(nal);
        }
        ...
        try {
            ...
            console.log("WebCodecs decode failed:", e);
            this.fail("decode failed");
            return false;
        }
    }
}
...
class CommandLink extends WebSocket {
    constructor(url, p) {
        ...
        this.imageClasses = []
        this.useNativeVideoDecoder = false
        this.nativeVideoDecoder = null
        if (this.videoPlayer != null
                && typeof this.videoPlayer.renderNativeFrame == "function"
                && NativeH264Decoder.isSupported()) {
            this.useNativeVideoDecoder = true;
            this.nativeVideoDecoder = new NativeH264Decoder(this.videoPlayer, 30, (reason) => {
                console.log("Fallback to wasm decoder:", reason);
                this.useNativeVideoDecoder = false;
                this.nativeVideoDecoder = null;
            });
        }
