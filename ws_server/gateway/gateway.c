...
static int gateway_send_init(int fd)
{
	uint8_t buf[GATEWAY_PROTO_INIT_SIZE];
	gateway_proto_init_info_t init_info;

	gateway_proto_default_init(&init_info);
	if (gateway_proto_write_init(buf, &init_info) != 0)
		return -1;
	return tcp_transport_write_all(fd, buf, sizeof(buf));
}
