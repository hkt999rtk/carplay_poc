...
Two Windows-facing release packages are maintained:

- source release
  - packages the current host sources, Ameba sources, docs, SDK/tool archives, and pristine-SDK overlay files
  - generated by:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\package_source_release.ps1
```

- binary release
  - packages the current Windows host executables, required DLLs, test media, and the latest Desktop `flash_is.bin`
  - generated by:

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\package_windows_test_release.ps1
```

Both packages include their own English `README.md` with usage instructions.
