    ...
    filterDetectedClass(osd) {
        if (osd.result.status == "ok") {
            ...
            for (let i=0; i<detection.length; i++) {
                let od = detection[i];
                if (this.filterClasses.includes(od.class)) {
...
updateFaceTablePlumerai = (resp) => {
    var genHTML = '<thead class="thead-dark"><tr><td>Face ID</td><td>Name</td><td></td></tr></thead>';
    genHTML += '<tbody>';
    for (let i=0; i<resp.length; i++) {
        genHTML += '<tr><td>' + resp[i].faceid + '</td><td>' + resp[i].name + '</td>';
        genHTML += '<td><button id="del_' + resp[i].faceid + '" type="button" class="btn btn-danger btn-sm"> Delete</button></td>';
        genHTML += '</tr>';
    }
    genHTML += '</tbody>';
    $("#facelibrary_table").html(genHTML);
    for (var i=0; i<resp.length; i++) {
        var id = "#del_" + resp[i].faceid;
        $(id).click((event) => {
            let faceid = event.currentTarget.id.substring(4);
            videoCmdLink.unregFace(faceid);
            videoCmdLink.getRegFace((r) => {
                updateFaceTablePlumerai(r);
            })
        });
    }
}

addMargin = (rect, marginW, marginH) => {
...
class CommandLink extends WebSocket {
    constructor(url, p) {
        ...
        this.onopen = () => {
            ...
            this.stopModel(["yolo4t", "retinaface", "mobilenetface", "yamnet", "object_tracking", "palm_detection", "hand_landmark", "plumerai"]);
            p.models = ["plumerai"];
        ...
        this.onmessage = (evt) => {
            if (evt.data instanceof ArrayBuffer) {
                ...
                switch (obj.type) {
                    case "get_model_status_plumerai":
                    ...
                    case "hand_gesture":
                        if (obj.type == "facial_recognition") {
                            // TODO: dirty code, check this later
    ...
    regFace(name) {
        this.send(JSON.stringify({cmd:"reg_face_plumerai", param: {"name": name}}));
    }
    ...
    unregFace(faceid) {
        ...
        this.send(JSON.stringify({cmd:"unreg_face_plumerai", param: {"faceid": faceid}}));
    }

    unregAllFace() {
        this.send(JSON.stringify({cmd: "unreg_allface_plumerai"}));
    }
    ...
    getRegFace(ready) {
        ...
        this.send(JSON.stringify({cmd:"get_reg_face_plumerai"}));
    }
    ...
    renameFace(faceid, newName) {
        ...
        this.send(JSON.stringify({cmd:"rename_face_plumerai", param: {"faceid": faceid, "name": newName}}));
    }

    saveFace() {
        this.send(JSON.stringify({cmd:"save_reg_face_plumerai"}));
    }
